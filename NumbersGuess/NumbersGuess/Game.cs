...
namespace NumbersGuess
{
    public partial class Game : Form
    {
        ...
        private void anOneTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            //e.KeyChar獲取按鍵值 !char.IsDigit判斷輸入的不是0~9
            if (!char.IsDigit(e.KeyChar) && !char.IsControl(e.KeyChar))
                e.Handled = true;//表示按下的資料不會輸入
        }

        private void anTwoTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsDigit(e.KeyChar) && !char.IsControl(e.KeyChar))
                e.Handled = true;
        }

        private void anThreeTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsDigit(e.KeyChar) && !char.IsControl(e.KeyChar))
                e.Handled = true;
        }

        private void anFourTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsDigit(e.KeyChar) && !char.IsControl(e.KeyChar))
                e.Handled = true;
        }

        #endregion 只能按數字鍵和功能鍵

        private void autoButton_Click(object sender, EventArgs e)
        {
            int[] autoRunRandomArray = new int[4];
            Random autoRunRandom = new Random();
            for (int a = 0; a < 4; a++)
            {
                autoRunRandomArray[a] = autoRunRandom.Next(1, 10);

                for (int b = 0; b < a; b++)
                {
                    while (autoRunRandomArray[a] == autoRunRandomArray[b])
                    {
                        b = 0;
                        autoRunRandomArray[a] = autoRunRandom.Next(1, 10);
                    }
            ...
            anOneTextBox.Text = autoRunRandomArray[0].ToString();
            anTwoTextBox.Text = autoRunRandomArray[1].ToString();
            anThreeTextBox.Text = autoRunRandomArray[2].ToString();
            anFourTextBox.Text = autoRunRandomArray[3].ToString();
